...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $checkEmailQuery = "SELECT jobseeker_ID, firstname FROM JOB_SEEKER_SIGNUP_INFO WHERE email = ?";
    $stmtCheckEmail = mysqli_prepare($conn, $checkEmailQuery);

    if ($stmtCheckEmail) {
        mysqli_stmt_bind_param($stmtCheckEmail, "s", $email);
        mysqli_stmt_execute($stmtCheckEmail);
        mysqli_stmt_bind_result($stmtCheckEmail, $jobseeker_ID, $firstname);
        ...
        if (mysqli_stmt_num_rows($stmtCheckEmail) > 0) {
            ...
            if ($stmtInsertRequest) {
                $user_type = "Job Seeker"; // Assuming jobseeker for this example

                mysqli_stmt_bind_param($stmtInsertRequest, "sss", $request_ID, $jobseeker_ID, $user_type);
                mysqli_stmt_execute($stmtInsertRequest);

                // Direction message for the user
                $displayedName = $firstname[0] . str_repeat('*', max(0, strlen($firstname) - 2)) . substr($firstname, -2);
