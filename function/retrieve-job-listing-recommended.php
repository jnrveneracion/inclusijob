...
$query1 = "WITH CombinedWords AS (
               SELECT
               JL.job_ID,
               CONCAT(
                    ESI.company_name, ' ', ESI.industry_sector, ' ', ESI.company_address, ' ', ESI.company_description, ' ', ESI.company_culture, ' ',
                    JL.job_title, ' ', JL.job_description, ' ', JL.qualifications, ' ', JL.job_location, ' ',
                    JL.employment_type, ' ', JL.benefits
               ) AS compiled_words,
               JL.job_title AS job_title
               FROM
               EMPLOYER_SIGNUP_INFO ESI
               LEFT JOIN
               JOB_LISTING JL ON ESI.company_ID = JL.employer_id

               UNION ALL

               SELECT
               NULL AS job_ID, -- Adjust this if the other SELECT doesn't have a job_ID column
               NULL AS compiled_words, -- Adjust this if the other SELECT doesn't have a compiled_words column
               JSCH.job_title AS job_title
               FROM
               JOB_SEEKER_SIGNUP_INFO JSSI
               LEFT JOIN
               JOB_SEEKER_EDUCATION_INFO JSEI ON JSSI.jobseeker_ID = JSEI.jobseeker_ID
               LEFT JOIN
               JOB_SEEKER_CAREER_HISTORY JSCH ON JSSI.jobseeker_ID = JSCH.jobseeker_ID
               LEFT JOIN
               JOB_SEEKER_SKILL JSS ON JSSI.jobseeker_ID = JSS.jobseeker_ID
               WHERE JSEI.jobseeker_ID = '$jobseeker_ID'
          ...
          FROM (
               SELECT
               job_ID,
               TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(compiled_words, ' ', numbers.n), ' ', -1)) AS word,
               job_title
               FROM
               CombinedWords
               JOIN (
               SELECT
                    (HUNDREDS.i * 100 + TENS.i * 10 + ONES.i + 1) AS n
               FROM
                    (SELECT 0 AS i UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9) AS ONES
               CROSS JOIN
               ...
               ) AS numbers ON CHAR_LENGTH(compiled_words) - CHAR_LENGTH(REPLACE(compiled_words, ' ', '')) >= numbers.n - 1
               WHERE TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(compiled_words, ' ', numbers.n), ' ', -1)) NOT IN (SELECT word FROM SimpleWords)
          ) AS words
          GROUP BY job_ID
...
$stmt1 = mysqli_prepare($conn, $query1);
 // Initialize an array to store job_IDs
...
// Query to retrieve job listings
...
$query = "SELECT *,
          ...
          ORDER BY FIELD(JL.job_id, $jobIdsString)";
...
if ($stmt === false) {
     ...
     if (mysqli_stmt_execute($stmt)) {
          ...
          while ($row = mysqli_fetch_assoc($result)) {
               ...
               if (strlen($qualifications_content) > 160) {
                    $qualifications_content = substr($qualifications_content, 0, 160) . '.... <span style="font-weight: 600;">continue reading</span>';
               }

               $savedStatusRow = $row['saved_status'];
               $saved_status = (($savedStatusRow === 1)) ? 'd-block' : 'd-none';
               $unsaved_status = (($savedStatusRow === 0)) ? 'd-block' : 'd-none';
                              ...
                              <div class="row head-section position-relative">
                                   ...
                                   <div class="position-absolute d-flex justify-content-end align-items-start mt-1">
                                        <div class="svg-container">
                                             <svg class="save-job first-svg ' . $unsaved_status . '" onclick="toggleSave(' . $itemCount . ')" id="save-job-' . $itemCount . '" id="job-' . $itemCount . '" job-listing-id="' . $row['job_id'] . '" job-seeker-id="' . $_SESSION['jobseeker_ID'] . '" employer-id="' . $row['company_ID'] . '" xmlns="http://www.w3.org/2000/svg" height="1.5em" viewBox="0 0 384 512"><path d="M0 48C0 21.5 21.5 0 48 0l0 48V441.4l130.1-92.9c8.3-6 19.6-6 27.9 0L336 441.4V48H48V0H336c26.5 0 48 21.5 48 48V488c0 9-5 17.2-13 21.3s-17.6 3.4-24.9-1.8L192 397.5 37.9 507.5c-7.3 5.2-16.9 5.9-24.9 1.8S0 497 0 488V48z"/></svg>
                                             <svg class="unsave-job second-svg ' . $saved_status . '" onclick="toggleUnSave(' . $itemCount . ')" id="unsave-job-' . $itemCount . '" job-listing-id="' . $row['job_id'] . '" job-seeker-id="' . $_SESSION['jobseeker_ID'] . '" employer-id="' . $row['company_ID'] . '" xmlns="http://www.w3.org/2000/svg" height="1.5em" viewBox="0 0 384 512"><path d="M0 48V487.7C0 501.1 10.9 512 24.3 512c5 0 9.9-1.5 14-4.4L192 400 345.7 507.6c4.1 2.9 9 4.4 14 4.4c13.4 0 24.3-10.9 24.3-24.3V48c0-26.5-21.5-48-48-48H48C21.5 0 0 21.5 0 48z"/></svg>
                                        </div>
                                   </div>
                              </div>
                              <div>
                              ...
                              <div>
                                   <div class="row head-section">
                                        ...
                                        <div class="col-1 d-flex justify-content-start align-items-center">
                                             ...
                                             <svg class="unsave-job second-svg ' . $saved_status . '" onclick="toggleUnSave(' . $itemCount2 . ')" id="unsave-job-' . $itemCount2 . '" job-listing-id="' . $row['job_id'] . '" job-seeker-id="' . $_SESSION['jobseeker_ID'] . '" employer-id="' . $row['company_ID'] . '" xmlns="http://www.w3.org/2000/svg" height="2.5em" viewBox="0 0 384 512"><path d="M0 48V487.7C0 501.1 10.9 512 24.3 512c5 0 9.9-1.5 14-4.4L192 400 345.7 507.6c4.1 2.9 9 4.4 14 4.4c13.4 0 24.3-10.9 24.3-24.3V48c0-26.5-21.5-48-48-48H48C21.5 0 0 21.5 0 48z"/></svg>
                                        </div>
                                   </div>
                                   <div>
                                        <div class="align-items-center">
                                             ...
                                             <div class="row">
                                                  <div class="col-12 col-lg-6">
                                                       ...
                                                       <div class="d-flex align-items-center">
                                                            ...
                                                            <p class="ms-2 mb-1 mt-1"><span style="font-weight: 500;">Email:</span>&nbsp' . $row['email'] . '</p>
                                                       </div>
                                                  </div>
                                                  <div class="col-12 col-lg-6">
                                                       <div class="d-flex align-items-center ' . ((!empty($row['company_website'])) ? 'd-flex' : 'd-none') . '">
                                                            ...
                                                            <p class="ms-2 mb-1 mt-1"><span style="font-weight: 500;">Website:</span>&nbsp' . $row['company_website'] . '</p>
                                                       </div>
                                                       <div class="d-flex align-items-center ' . ((!empty($row['company_facebook'])) ? 'd-flex' : 'd-none') . '">
                                                            ...
                                                            <p class="ms-2 mb-1 mt-1"><span style="font-weight: 500;">Facebook:</span>&nbsp' . $row['company_facebook'] . '</p>
                                                       </div>
                                                       <div class="d-flex align-items-center ' . ((!empty($row['company_linkedin'])) ? 'd-flex' : 'd-none') . '">
                                                            ...
                                                            <p class="ms-2 mb-1 mt-1"><span style="font-weight: 500;">Linkedin:</span>&nbsp' . $row['company_linkedin'] . '</p>
                                                       </div>
                                                       <div class="d-flex align-items-center ' . ((!empty($row['company_twitter'])) ? 'd-flex' : 'd-none') . '">
                                                            ...
                                                            <p class="ms-2 mb-1 mt-1"><span style="font-weight: 500;">Twitter:</span>&nbsp' . $row['company_twitter'] . '</p>
                                                       </div>
                                                  </div>
                                             </div>     
                                        </div>
