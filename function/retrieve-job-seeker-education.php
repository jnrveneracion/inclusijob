     ...
     if ($stmt === false) {
          ...
          if (mysqli_stmt_execute($stmt)) {
               ...
               while ($row = mysqli_fetch_assoc($result)) {        
                    ...
                    <div class="info-body me-lg-2 me-0 ms-lg-2 me-2 ms-2 mt-lg-0 mt-3 mb-lg-3 mb-0" style="min-height: 345px;"> 
                         ...
                         <p class="info-section p-0 m-0 ' . (empty(trim($start_year)) ? 'd-block' : 'd-block') . '"><span class="info-label">Start Year: </span><span class="info-data">' . $row['start_year'] . '</span></p>
                         <p class="info-section p-0 m-0 ' . (empty(trim($grad_year)) ? 'd-block' : 'd-block') . '"><span class="info-label">Graduation Year: </span><span class="info-data">' . $row['graduation_year'] . '</span></p>
                         <p class="info-section p-0 m-0 ' . (empty(trim($course_highlights)) ? 'd-block' : 'd-block') . '"><span class="info-label">Course Highlights: </span><br><p class="info-data" style="max-height: 90px; overflow: scroll; min-height: 90px;">' . $row['course_highlights'] . '</p></p>
                         <div class="d-flex justify-content-end" style="' . ((empty(trim($start_year)) or empty(trim($grad_year))) ? 'margin-top: -5px;' : 'margin-top: -5px;') . '">
