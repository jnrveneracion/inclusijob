<?php
     ...
     $query = "SELECT 
               ...
               WHERE REQUEST_ACCOUNT_DELETION.user_type = 'Employer';";
...
if (mysqli_num_rows($result) > 0) {
     ...
     while ($row = mysqli_fetch_assoc($result)) {
          $action = !empty($row['deletion_status']) ? '
               ...
               : 
               '<button class="btn btn-light btn-outline-secondary border-1 btn-delete-company-data" style="font-size: 10px;" data-request-deletion-id="' . $row['request_password_id'] . '" data-employer-id="' . $row['user_ID'] . '">Delete User Data</button>'
               ;
